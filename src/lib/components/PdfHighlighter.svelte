...
<script lang="ts">
    ...
    import type {Highlight, HighlightBindings, PdfSelection, Tip, SearchOptions, ViewportPosition, Content,} from "$lib/types.ts";
    ...
    let enableAreaSelection: (e: MouseEvent) => boolean = (event) => event.altKey || selectedTool === 'area_selection';
	...
	onMount(() => {
	    ...
	    let doc_init = setInterval(() => {
            ...
            removeWheelHandler = on(viewerRef!.container, 'wheel', (event) => {
                    if (event.ctrlKey) {
                        event.preventDefault()
                        setPdfScaleValue((typeof pdfScaleValue.val == 'number') ? pdfScaleValue.val - (event.deltaY / 1000) : 1.0); //TODO: get actual scale?
                        //handleScaleValue();
                    }
                }, { passive: false }
            );
    ...
    const handleMouseDown: PointerEventHandler = (event) => {
        ...
        if (selectedTool === 'hand' || 
            (
                (!event.target.closest("span") && !event.target.closest('.resizers')) 
                && 
    ...
    const scrollToHighlight = (highlight: Highlight) => {
        ...
        const pageViewport = viewerRef!.getPageView(
            pageNumber - 1,
        ).viewport;
